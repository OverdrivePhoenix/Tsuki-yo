...
export const Dashboard: React.FC<DashboardProps> = ({ onPreview }) => {
  ...
  const fetchSubtitlesFromYoutubeById = async (videoId: string): Promise<{ absoluteStart: number; text: string }[] | null> => {
    try {
      const streamData = await queryPipedInstances(`/streams/${videoId}`);
      if (!streamData.subtitles || streamData.subtitles.length === 0) return null;
      ...
      if (!bestSub || !bestSub.url) return null;
      ...
      console.warn(`Failed to fetch subtitles for video ${videoId} from all Piped instances:`, e);
    }
    return null;
  };

  // Helper to search YouTube for videos and format them for list selection
  const searchYoutubeVideos = async (query: string): Promise<any[]> => {
    try {
      const searchData = await queryPipedInstances(`/search?q=${encodeURIComponent(query + " lyrics")}&filter=videos`);
      ...
      return videos.map((video: any) => {
        ...
        return {
          ...
          syncedLyrics: "[YOUTUBE TRANSCRIPT AVAILABLE]"
        };
      });
    } catch (e) {
      console.warn("All Piped instances failed YouTube search:", e);
      return [];
    }
  };

  // Helper to search YouTube for videos and extract subtitles directly
  const fetchSubtitlesFromYoutube = async (artist: string, trackName: string): Promise<{ absoluteStart: number; text: string }[] | null> => {
    ...
    try {
      const searchData = await queryPipedInstances(`/search?q=${encodeURIComponent(searchQuery)}&filter=videos`);
      ...
      return await fetchSubtitlesFromYoutubeById(videoId);
    } catch (e) {
      console.warn(`Failed to search or fetch subtitles for ${searchQuery} from all Piped instances:`, e);
      return null;
    }
  };
  ...
  const handleLyricsSearch = async () => {
    ...
    try {
      ...
      const ytPromise = searchYoutubeVideos(lyricsSearchQuery);
